...
// Exit if debug drawing is turned off or the target doesn't exist
if (!draw_debug_box || !instance_exists(target)) {
    exit;
}

// Get the camera's center position in the room
...
// Calculate the coordinates for the dead zone box around the camera's center
...
// Draw the rectangle outline in the game world
draw_set_color(c_lime);
draw_set_alpha(0.5);

draw_rectangle(_box_left, _box_top, _box_right, _box_bottom, true);

// Good practice: Reset draw settings
draw_set_alpha(1.0);
